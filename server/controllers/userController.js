...
const handleEditBio = async (req, res) => {
  ...
  if (!user || !bio)
    res.status(400).json({ msg: "Please enter a user and a bio" });
...
const handleEditUsername = async (req, res) => {
  ...
  if (!currentUsername || !newUsername)
    return res
      .status(400)
      .json({ msg: "Please enter a currentUsername and a newUsername" });
  const foundCurrentUsername = await User.findOne({
...
const handleChangePass = async (req, res) => {
  ...
  if (!user || !oldPass || !newPass)
    return res
      .status(400)
      .json({ msg: "Please enter a user, your old pass, and a new pass" });
  const foundUser = await User.findOne({ username: user }).exec();
  if (!foundUser) return res.status(400).json({ msg: "User does not exist" });
  ...
  if (confirmOldPass && oldPass === newPass)
    return res.status(400).json({ msg: "Password already in use" });
  try {
